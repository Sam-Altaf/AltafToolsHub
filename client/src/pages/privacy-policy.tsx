...
const sections = [
  ...
  },
  {
    ...
    content: [
      ...
      "**Necessary Cookies (Always Active)**: Session management cookies that expire when you close your browser, security tokens for CSRF protection, cookie consent preferences (stored for 365 days), and usage tracking cookies for freemium quota management.",
      "**Usage Tracking Cookies**: To implement our freemium model, we use cookies to track: daily usage counts per tool, monthly usage quotas, last usage timestamps for rate limiting, remaining free tier allowances, and whether paid features should be activated. These cookies are essential for service operation and cannot be disabled while using free tier features.",
      "**Analytics Cookies (Optional)**: We use Google Analytics to understand how visitors use our website. This includes anonymous usage statistics, aggregated page view tracking, tool usage metrics (which tools are used, not what you do with them), performance monitoring for page load times, and user behavior patterns. We may also use analytics data for monetization purposes including understanding user patterns for advertising optimization and creating anonymized market insights. Google Analytics uses cookies to collect this information anonymously.",
      "**Functional Cookies (Optional)**: These remember your theme preference (light/dark mode), language settings if applicable, default tool preferences, recently used tools for quick access, and freemium tier status.",
      "**Local Storage Usage**: We store usage quota information in local storage to enforce freemium limits. We never store file data in local storage. Only UI preferences, non-sensitive settings, and usage counters are saved. All local storage can be cleared via browser settings, and we provide a 'Clear All Data' option (which will reset your free usage quotas).",
      ...
      "**Cookie Control**: You can manage all cookies through our consent banner, your browser settings, or by using Do Not Track signals which we fully honor. Note that disabling usage tracking cookies may limit your access to free tier features."
    ]
  },
  {
    icon: Users,
    ...
    content: [
      ...
      "**Usage Tracking for Freemium Model**: We track usage metrics to implement and manage our freemium model, including: number of daily tool uses per browser session, cumulative usage counts for quota enforcement, timestamps of tool usage for rate limiting, tool-specific usage patterns for pricing tiers, remaining free tier allowances, and usage frequency to determine when paid features activate. This data is stored locally in your browser and/or anonymously on our servers solely for usage limit enforcement.",
      "**Technical Information**: JavaScript error logs (not your data), performance metrics like page load times, feature usage statistics (which buttons are clicked), browser capabilities for feature detection, and freemium quota status.",
      ...
      "**Aggregation and Anonymization**: All analytics data is aggregated and anonymized. We cannot identify individual users from this data, and we don't attempt to re-identify anonymized data. Usage limits are enforced per browser session without personal identification."
    ]
  },
  {
    icon: Lock,
  ...
  },
  {
    ...
    content: [
      ...
      "**Analytics Service (Google Analytics)**: We use Google Analytics to understand website usage patterns and improve our services. Google Analytics collects anonymous data including page views, session duration, bounce rates, browser and device information, geographic region (country/city level only), traffic sources, and user flow through our site. We have configured Google Analytics with IP anonymization enabled, no remarketing features, no advertising features, no demographic data collection, and no user-ID tracking. You can opt-out using the Google Analytics Opt-out Browser Add-on.",
      ...
      "**Google Analytics Data Collection**: We use Google Analytics with privacy-protective settings to understand how our website is used. Google Analytics collects: page views and page titles, time spent on each page, links clicked and downloads, search terms used to find our site, general geographic location (country/city), browser type and version, device type (mobile/desktop), screen resolution, and operating system. We have disabled all advertising features, remarketing, demographic reports, and user-ID tracking in Google Analytics.",
      "**How to Opt-Out of Analytics**: You have multiple options to opt-out of Google Analytics tracking: Install the Google Analytics Opt-out Browser Add-on (https://tools.google.com/dlpage/gaoptout), use browser privacy modes (Incognito/Private browsing), enable Do Not Track in your browser (we honor DNT signals), or use ad-blocking extensions that block analytics scripts. Your opt-out choice is respected across all sessions.",
      "**Advertising & Monetization Disclosure**: We reserve the right to display advertisements, affiliate links, sponsored content, and promotional materials at any time without prior notice to support our free tools and cover operational costs. This includes but is not limited to display ads, affiliate marketing links, sponsored recommendations, partner product placements, commission-based promotions, and contextual advertising. We maintain our commitment to not sharing your personal file data with advertising networks.",
      "**Service Sustainability & Monetization**: Our tools are currently free to use, however we reserve the right to introduce premium features, paid tiers, subscription models, usage limitations (such as 4 free uses per day), or any other monetization methods at any time without notice. Changes may be implemented immediately when server costs cannot be covered, hosting providers threaten suspension, or financial sustainability is at risk. We may implement daily/monthly quotas, credit systems, pay-per-use models, or any combination of usage restrictions without notice. The 'Free Forever' designation refers to our current model and is subject to immediate change without notice.",
      "**Affiliate Marketing & Sponsored Content**: We may participate in affiliate marketing programs and display sponsored content without notice. This includes earning commissions from qualifying purchases through affiliate links, featuring sponsored tool recommendations, displaying partner service promotions, and implementing commission-based referral systems. Your file data is never shared with affiliate partners.",
      "**What We DON'T Use Currently**: No Facebook Pixel or social media tracking. No session recording tools. No heat mapping beyond basic analytics. No A/B testing tools that track individuals. No customer data platforms or CRMs. No cross-site tracking or fingerprinting.",
  ...
  },
  {
    ...
    content: [
      ...
      "**Policy Update Procedures**: This policy is reviewed quarterly and updated as needed. Changes are effective immediately upon posting unless otherwise stated. We may announce significant changes via website banner but are not obligated to provide advance notice. Previous policy versions are available upon request.",
      ...
      "**Data Retention Periods**: Analytics data: Aggregated and kept for 12 months. Error logs: 30 days then permanently deleted. Support emails: 90 days then permanently deleted. No file data ever retained. Cookie preferences: 365 days. Anonymized usage data may be retained indefinitely for business analytics and monetization purposes. All personal data deletion is permanent and irreversible.",
